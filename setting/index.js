...
async function main() {
	...
	const normal_screenshot = document.getElementById("normal-screenshot");
	const all_screenshot = document.getElementById("all-screenshot");
	// 界面文字设置
	...
	normal_screenshot.innerHTML = chrome.i18n.getMessage('normalScreenshot')+'(Alt+Shift+W)';
	all_screenshot.innerHTML=chrome.i18n.getMessage('allScreenshot')+'(Alt+Shift+E)';
	// 存储用户设置和回显设置逻辑
	...
	// 截图
	normal_screenshot.addEventListener('click', () => {
		chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
			// 通知 content.js 区域截图
	...
	// 长截图
	all_screenshot.addEventListener('click', () => {
		chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
			// 通知 content.js 区域截图
